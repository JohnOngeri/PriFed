    ...
    def _engineer_payment_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in card_cols:
            if df[col].dtype in ['int64', 'float64']:
                ...
                df[f'{col}_first_digit'] = pd.to_numeric(df[col].astype(str).str[0], errors='coerce')
...
def advanced_preprocessing_pipeline(df: pd.DataFrame, is_training: bool = True, 
    ...
    if is_training and len(df_processed.columns) > 100:
        ...
        # Ensure test has ALL selected features (create missing ones as 0)
        for f in selected_features:
            if f not in df_processed.columns:
                df_processed[f] = 0

        # Drop any extra columns and keep exact order
...
def _advanced_missing_value_imputation(df: pd.DataFrame, numeric_features: List[str], 
    ...
    if categorical_features:
        if is_training:
            ...
            artifacts['categorical_imputer_features'] = list(categorical_features)
        else:
            if 'categorical_imputer' in artifacts:
                cat_features = [col for col in categorical_features if col in df_imputed.columns]
                if cat_features:
                    fitted_cats = artifacts.get('categorical_imputer_features', cat_features)

                    # Ensure all fitted columns exist in df (create missing ones as NaN)
                    for c in fitted_cats:
                        if c not in df_imputed.columns:
                            df_imputed[c] = np.nan

                    # Transform only fitted columns, in the same order
                    df_imputed[fitted_cats] = artifacts['categorical_imputer'].transform(df_imputed[fitted_cats])
                ...
                cat_features = [col for col in categorical_features if col in df_imputed.columns]
                if cat_features:
                    # Use the exact columns seen during fit (SimpleImputer validates feature names)
                    fitted_cats = artifacts.get('categorical_imputer_features', cat_features)
    ...
    # FINAL SAFETY NET: SelectKBest cannot handle NaNs.
    # Some columns may be skipped by KNN/Iterative due to performance limits,
    # so we guarantee all remaining NaNs are filled here.
    if numeric_features:
        df_imputed[numeric_features] = df_imputed[numeric_features].replace([np.inf, -np.inf], np.nan)
        df_imputed[numeric_features] = df_imputed[numeric_features].apply(lambda s: s.fillna(s.median()))
    ...
    if is_training:
        ...
        if 'scaler' in artifacts:
            ...
            if available_features:
                ...
                # Use the exact feature names seen during fit (sklearn validates feature names)
                fitted_features = list(getattr(scaler, 'feature_names_in_', []))
                if not fitted_features:
                    fitted_features = [c for c in numeric_features if c in df_scaled.columns]

                # Ensure all fitted features exist at inference time
                for c in fitted_features:
                    if c not in df_scaled.columns:
                        df_scaled[c] = 0

                # Transform in the same order as fit
                df_scaled[fitted_features] = scaler.transform(df_scaled[fitted_features])
