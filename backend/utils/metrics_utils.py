...
def compute_classification_metrics(
    ...
    threshold: float = 0.5,
    include_score_samples: bool = False,
    max_score_samples: int = 2000,
) -> Dict[str, float]:
    """
    ...
    # Score distribution diagnostics (useful for collapse debugging)
    y_score = np.asarray(y_pred_proba, dtype=float)
    metrics_score = {
    ...
    # Basic metrics (include class counts + score diagnostics)
    n_pos = int(np.sum(y_true == 1))
    n_neg = int(np.sum(y_true == 0))
    metrics = {
        'n_pos': n_pos,
        'n_neg': n_neg,
        'true_pos_rate': float(n_pos / max(len(y_true), 1)),
    ...
    # Log loss (guard single-class + older sklearn)
    if len(np.unique(y_true)) > 1:
        try:
    ...
    metrics['pred_negative_rate'] = float(1 - np.mean(y_pred))
    ...
    # Debug-only: include pooled score samples (privacy-sensitive)
    if include_score_samples:
        n = min(len(y_true), max_score_samples)
        metrics['score_samples_n'] = int(n)
        metrics['y_true_json'] = json.dumps(y_true[:n].astype(int).tolist())
        metrics['y_score_json'] = json.dumps(y_pred_proba[:n].astype(float).tolist())
