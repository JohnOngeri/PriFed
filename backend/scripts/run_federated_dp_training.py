"""
Run federated learning with differential privacy.
"""

import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import logging
from datetime import datetime
import argparse
import json

from utils.data_utils import load_config, prepare_local_datasets_for_banks, prepare_global_test_set
from utils.fl_utils import run_federated_training

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

def main():
    parser = argparse.ArgumentParser(description='Run federated training with DP')
    parser.add_argument('--config', type=str, default='configs/config.yaml')
    parser.add_argument('--epsilon', type=float, help='Privacy budget')
    args = parser.parse_args()
    
    config = load_config(args.config)
    config['differential_privacy']['enabled'] = True
    if args.epsilon:
        config['differential_privacy']['target_epsilon'] = args.epsilon
    
    os.makedirs('logs', exist_ok=True)
    os.makedirs('results', exist_ok=True)
    
    logger.info("Starting federated training with differential privacy")
    
    # Prepare data
    bank_datasets = prepare_local_datasets_for_banks(config)
    X_test, y_test = prepare_global_test_set(config)
    
    # Run federated training with DP
    results = run_federated_training(config, bank_datasets, (X_test, y_test))
    
    # Save results
    with open('results/federated_dp_results.json', 'w') as f:
        json.dump({
            'experiment_type': 'federated_dp',
            'config': config,
            'results': results,
            'timestamp': datetime.now().isoformat()
        }, f, indent=2, default=str)
    
    logger.info("Federated DP training completed")

if __name__ == "__main__":
    main()